DECLARE
	...
	SELECT activityguid, fieldindex FROM Activity_Info_Index;
	 ...
	 v_PartialPaymentDaysRemaining number;
BEGIN
    ...
    LOOP
		...
		FOR i IN 0..(v_fieldindex - 1) LOOP
			...
			SELECT count(1) into v_PartialPaymentDaysRemaining FROM AsActivityMultivalueField 
			WHERE FieldName = 'PartialPaymentDaysRemaining' AND FieldIndex = i AND ActivityGuid = v_activityguid;
			-- Check if the record PartialPaymentDaysRemaining exists
			IF v_PartialPaymentDaysRemaining = 0 THEN		
				...
				INSERT INTO ASACTIVITYMULTIVALUEFIELD (ActivityGuid, FieldName, FieldIndex, FieldTypeCode, FloatValue)
					VALUES(v_activityguid, 'PartialPaymentDaysRemaining', i, '04', '0');
			END IF;
