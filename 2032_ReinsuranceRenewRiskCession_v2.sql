BEGIN
  ...
  WITH act_base AS (
    ...
    JOIN AsTransaction TXN ON TXN.TransactionGUID = ACT.TransactionGUID
		  AND TXN.TransactionName = 'ReinsuranceRenewRiskCession'
    ...
    WHERE ACT.StatusCode IN ('01','02','09')
  ),

  seg_rci_agg AS (
    SELECT
      ROLE_REIN.SegmentGUID,
      LISTAGG(DISTINCT RF_RCI.TextValue, '') WITHIN GROUP (ORDER BY RF_RCI.TextValue) AS SegmentRCI
  ...
  WHERE NOT EXISTS (
    ...
    JOIN AsTransaction TXN_CHECK ON TXN_CHECK.TransactionGUID = ACT_CHECK.TransactionGUID
		  AND TXN_CHECK.TransactionName = 'ReinsuranceRenewRiskCession'
    WHERE ACT_CHECK.ActivityGUID = RV.ActivityGUID
		  AND ACT_CHECK.StatusCode IN ('01','02','09')
  );
END;
/