...
BEGIN
    ...
    LOOP
		...
		FOR i IN 0..(v_fieldindex - 1) LOOP
			...
			SELECT count(1) into v_SettlementTypeID FROM AsActivityMultivalueField 
			WHERE FieldName = 'SettlementTypeID' AND FieldIndex = i AND ActivityGuid = v_activityguid;
			-- Check if the record SettlementTypeID exists
			IF v_SettlementTypeID = 0 THEN		
				...
				INSERT INTO ASACTIVITYMULTIVALUEFIELD (ActivityGuid, FieldName, FieldIndex, FieldTypeCode, TextValue)				
					VALUES(v_activityguid, 'SettlementTypeID', i, '02', NULL);
			END IF;
